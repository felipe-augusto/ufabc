...
class IntervaloBytes(PredicadoAbstrato):
    def __init__(self, minimo, maximo):
        super(IntervaloBytes, self).__init__()
        self._min = int(minimo)
        self._max = int(maximo)

    def verdadeiro(self, um_string):
        info = um_string.strip().split()
        return self._min <= int(info[3]) <= self._max


class PorCodigo(PredicadoAbstrato):
    def __init__(self, codigo):
        super(PorCodigo, self).__init__()
        self._codigo = codigo

    def verdadeiro(self, um_string):
        info = um_string.strip().split() 
        return info[0].find(self._codigo) >= 0